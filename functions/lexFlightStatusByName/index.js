...
module.exports.handler = (event, context, callback) => {
  ...
  let departureCity = event.currentIntent.slots.departure_city;
  let destinationCity = event.currentIntent.slots.destination_city;

  getFlights(departureCity, destinationCity).then(function(flights){
    let lastFlight = flights.length > 0 ? flights[0] : null;

    if (lastFlight) {
      let flightNumber = astFlight.flight.identification.number.default;
      getRecentFlights(flightNumber).then(function(recentFlightRecords){
        ...
        let nextFlight = recentFlightRecords['flightRecords'][recentFlightRecords['flightRecords'].length - 1];
        console.log('Last flight: ' + JSON.stringify(lastFlight, null, 2));
        let content = nextFlight ? createResponseContent(nextFlight) : `Flight with number: ${flightNumber} not found.`;
      ...
      const response = {
        "dialogAction": {
          ...
          "message": {
            "contentType": "PlainText",
            "content": `Flight from ${departureCity} to ${destinationCity} not found.`,
          }
        }
      };
    ...
    let content = lastFlight ? createResponseContent(lastFlight.flight) : `Flight from ${departureCity} to ${destinationCity} not found.`;
  });
};
